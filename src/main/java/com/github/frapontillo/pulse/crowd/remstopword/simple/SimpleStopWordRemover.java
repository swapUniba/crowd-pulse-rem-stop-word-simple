...
/**
 * Stop word remover plugin, marks tokens, tags and categories as stop words according to some stop
 * word files.
 *
...
public class SimpleStopWordRemover extends StopWordRemover<StopWordConfig> {
    ...
    // maps of dictionaries: key is language, value is list of filenames for the specific element
    // (or generic for all)
    ...
    @Override protected boolean isTokenStopWord(String token, String language,
                                                StopWordConfig stopWordConfig) {
        List<String> tokenDictionariesNames = union(getAllDictionaries(stopWordConfig, language),
                getTokenDictionaries(stopWordConfig, language));
        return isStopWord(token, tokenDictionariesNames);
    }

    @Override
    protected boolean isTagStopWord(String tag, String language, StopWordConfig stopWordConfig) {
        List<String> tagDictionariesNames = union(getAllDictionaries(stopWordConfig, language),
                getTagDictionaries(stopWordConfig, language));
        return isStopWord(tag, tagDictionariesNames);
    }

    @Override protected boolean isCategoryStopWord(String category, String language,
                                                   StopWordConfig stopWordConfig) {
        List<String> categoryDictionariesNames = union(getAllDictionaries(stopWordConfig, language),
                getCategoryDictionaries(stopWordConfig, language));
    ...
    @Override protected void processMessage(Message message, StopWordConfig stopWordConfig) {
        ...
        // for each element, reset the "stop word" property
        // by looking up the word in the proper dictionary
        ...
        if (stopWordConfig.mustStopTokens()) {
            ...
            if (tokens != null) {
                tokens.forEach(token -> token
                        .setStopWord(isTokenStopWord(token.getText(), language, stopWordConfig)));
            }
        }
            ...
            if (tags != null) {
                tags.forEach(tag -> {
                    ...
                    if (stopWordConfig.mustStopCategories()) {
                        ...
                        if (categories != null) {
                            categories.forEach(cat -> cat.setStopWord(
        ...
        if (!dictionaries.containsKey(fileName)) {
            ...
            try {
                List<String> lines = IOUtils.readLines(
                        getClass().getClassLoader().getResourceAsStream(fileName),"ISO-8859-1");
